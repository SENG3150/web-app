describe('DomainExpertIndexRoute', function(){
    ...
    it('should map index state  to correct url', function () {
        state = $state.get('domainExpert-index');
        expect(state.url).toEqual('/');
    });

    it('should map index state  to correct template url', function () {
        state = $state.get('domainExpert-index');
        expect(state.templateUrl).toEqual('views/domainExpert/index/index.html');
    });

    it('should map index state to use the right controller', function () {
        state = $state.get('domainExpert-index');
        expect(state.controller).toEqual('DomainExpertIndexControllerIndex');
    });
});
